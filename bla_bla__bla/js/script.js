  ...
  function postData(form) {   // form - данные от пользователя(если я правильно понял)
    form.addEventListener('submit', (e) => {  // событие submit будет срабатывать тогда, когда мы будем пытаться отрправить форму
      ...
      statusMessage.style.cssText = `
        display: block;
        margin: 0 auto;
      `; // пока сервер будет грузит данные мы просто выведим на экран с надписью loading...
      ...
      request.open('POST', 'server.php');  //  сбор и подгодовка данных для отправки на сервер
      // при использовании formData  заголовок не нужен(только тогда когда мы используем JSON используется заголовок)
      request.setRequestHeader('Content-type', 'application/json'); //  создаем заголовок которой будет говорить серверу, а что имено приходит
      ...
      const json = JSON.stringify(object); // превращает обычный объект в объект JSON формата
      ...
      request.addEventListener('load', () => {
        if (request.status === 200) {
          ...
          statusMessage.remove();// удаляет наше сообщение(спинер)
        } else {
